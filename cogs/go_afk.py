...
class GoAfk(commands.Cog):
    ...
    async def goafk(
        ...
        quiet: Option(
            str,
            description="Do you the bot to not announce your being AFK?",
            required=True,
            choices=["Off", "On"],
        ),
        status: Option(
            str,
            description="Enter the reason for being afk (optional)",
            max_length=1024,
            required=False,
        ),
        time_back: Option(
            str,
            description="Describe when you plan on being back (optional)",
            max_length=1024,
            required=False,
        ),
    ):
        ...
        log.debug(
            f"Received afk command from user {ctx.user.name}, in channel {ctx.channel.name},"
        ...
        query = (
            "INSERT OR REPLACE INTO Afk (usr, status, time_back, quiet, time, channel)"
        ...
        afk = discord.embeds.Embed(
            title=f"✅ {ctx.user.nick} is now afk", color=discord.Color.green()
        )
