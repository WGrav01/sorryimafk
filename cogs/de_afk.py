...
class DeAfk(commands.Cog):
    ...
    async def deafk(self, ctx,
                    quiet: Option(str, description='Do you the bot to not announce your coming back?',
                                  required=True, choices=['Off', 'On'])):

        log.debug(f'Received de-afk command from user {ctx.user.name}, in channel {ctx.channel.id},'
                  f' quiet: {quiet}')
        ...
        try:
            ...
            not_afk = discord.Embed(title='Error: you are not AFK', color=discord.Color.red())
            await db.close()
        ...
        log.debug(f'start_str: {result[0][4]}, end_str: {time.now()}, duration: {afk_duration}')
        afk.add_field(name='You have been away for',
                      value=afk_duration, inline=False)
